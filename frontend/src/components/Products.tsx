          ...
          <button 
            onClick={() => {
              const csvContent = [
                ...
                ...products.map(p => [p.id, p.name, p.description, p.price, p.stockQuantity])
              ].map(e => e.join(",")).join("\n");
          ...
          {products.map(product => (
            <div key={product.id} className="relative group bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 p-6 rounded-2xl shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
              ...
              <button 
                ...
                className="absolute top-4 right-14 p-2 bg-slate-900/80 hover:bg-indigo-600 text-slate-300 hover:text-white rounded-lg opacity-0 group-hover:opacity-100 transition-all shadow-lg backdrop-blur-md"
                title="Edit Product"
              >
              ...
              <button 
                ...
                className="absolute top-4 right-4 p-2 bg-slate-900/80 hover:bg-rose-600 text-slate-300 hover:text-white rounded-lg opacity-0 group-hover:opacity-100 transition-all shadow-lg backdrop-blur-md"
